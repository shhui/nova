...
class VMwareVMOpsTestCase(test.NoDBTestCase):
    ...
    def test_rename_virtualmachine(self):
        def fake_call_method(module, method, *args, **kwargs):
            if method == 'ReconfigVM_Task':
                self.assertEqual('test_vmware', kwargs['spec'].name)
            return 'fake_rename_virtualmachine'

        with contextlib.nested(
            mock.patch.object(self._session, '_wait_for_task'),
            mock.patch.object(self._session, '_call_method', fake_call_method)
        ) as (_wait_for_task, _call_method):
            self._vmops.rename_virtualmachine(None, self._instance)
            _wait_for_task.assert_has_calls([
                   mock.call('fake_rename_virtualmachine')])
