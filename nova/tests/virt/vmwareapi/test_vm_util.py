...
class VMwareVMUtilTestCase(test.NoDBTestCase):
    ...
    def test_get_hosts_ref_no_hosts_in_cluster(self):
        self.assertRaises(exception.NoValidHost,
                          vm_util.get_hosts_ref,
                          fake_session(""), 'fake_cluster')

    def test_get_hosts_ref(self):
        hosts = ['host1', 'host2']
        hosts = fake._convert_to_array_of_mor(hosts)
        prop_dict = {'host': hosts}

        session = fake_session()

        def fake_call_method(*args):
            if "get_dynamic_property" in args:
                return prop_dict['host']
        with mock.patch.object(fake_session, '_call_method',
                               fake_call_method):
            result = vm_util.get_hosts_ref(session, "cluster1")
            self.assertEqual(result[0], "host1")
            self.assertEqual(result[1], "host2")
    ...
    def test_get_vm_rename_spec(self):
        fake_instance = {'id': 7, 'name': 'fake!',
                         'display_name': 'test_vmware',
                         'uuid': 'bda5fb9e-b347-40e8-8256-42397848cb00',
                         'vcpus': 2, 'memory_mb': 2048}
        result = vm_util.get_vm_rename_spec(fake.FakeFactory(),
                                            fake_instance)
        expected = {'memoryMB': 2048,
                    'numCPUs': 2,
                    'display_name': 'test_vmware',
                    'obj_name': 'ns0:VirtualMachineConfigSpec'}
        self.assertEqual(expected['display_name'], result.name)
