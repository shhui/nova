...
class VMwareVMOps(object):
        ...
        def _get_vif_infos():
            ...
            for vif in network_info:
                ...
                network_ref = vmwarevif.get_network_ref(self._session,
                                                        ...
                                                        vif,
                                                        self._is_neutron)
                name = network_ref.get('name', None)
                if name is not None and (not network_name == name):
                    network_name = name
        ...
        if not ebs_root:
            ...
            if not (self._check_if_folder_file_exists(ds_browser,
                ...
                if not is_iso:
                    if disk_type != "sparse":
                        ...
                        ds_util.mkdir(self._session,
                                      ...
                                      dc_info.ref)
                        # NOTE(xg.song):For vmware template dummy image, we can
                        # copy the virtual disk from the template,
                        ...
                        if template_name is not None:
                            # We need to make target directory existed
                            ds_util.mkdir(self._session,
                                          ds_util.build_datastore_path(
                                              data_store_name, upload_folder),
                                          dc_info.ref)
                ...
                if template_name is None:
                    ...
                    # For vmware dummy template, copy from template
                    template_vmdk_file_name = '%s/%s.%s' % (template_name,
                                                            template_name,
                                                            file_type)
                    template_vmdk_file_path = ds_util.build_datastore_path(
                        data_store_name, template_vmdk_file_name)
                    _copy_virtual_disk(template_vmdk_file_path, upload_path)

                if not is_iso and disk_type == "sparse":
                    # Copy the sparse virtual disk to a thin virtual disk.
                    disk_type = "thin"
                    # For vmware dummy template, we have copied it
                    if template_name is None:
    ...
    def rescue(self, context, instance, network_info, image_meta):
        ...
        (vmdk_path, adapter_type,
         disk_type) = vm_util.get_vmdk_path_and_adapter_type(
                hardware_devices, instance=instance)
    ...
    def rename_virtualmachine(self, context, instance):
        client_factory = self._session._get_vim().client.factory
        vm_ref = vm_util.get_vm_ref(self._session, instance)
        vm_rename_spec = vm_util.get_vm_rename_spec(client_factory, instance)
        reconfig_task = self._session._call_method(
                                            self._session._get_vim(),
                                            "ReconfigVM_Task", vm_ref,
                                            spec=vm_rename_spec)
