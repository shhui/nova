...
def get_vm_rename_spec(client_factory, instance):
    """Provides rename for a VM spec."""
    rename_spec = client_factory.create('ns0:VirtualMachineConfigSpec')
    rename_spec.name = str(instance['display_name'])
    return rename_spec
...
def get_vmdk_path_and_adapter_type(hardware_devices, instance=None):
...
def get_add_dvswitch_port_group_spec(client_factory, port_group_name, vlan_id):
    """Builds the virtual switch port group add spec."""
    ...
    vswitch_port_group_spec.defaultPortConfig.blocked = blocked
    return vswitch_port_group_spec
...
def get_hosts_ref(session, cluster=None):
    """Get reference to a host within the cluster specified."""
    host_mors = []
    if cluster is None:
        results = session._call_method(vim_util, "get_objects",
        ...
        for result in results.objects:
            host_mors.append(result.obj)
    else:
        host_rets = session._call_method(vim_util, "get_dynamic_property",
                                         cluster, "ClusterComputeResource",
                                         "host")
        if not host_rets or not host_rets.ManagedObjectReference:
            msg = _('No host available on cluster')
            raise exception.NoValidHost(reason=msg)
        for host_ret in host_rets.ManagedObjectReference:
            host_mors.append(host_ret)
