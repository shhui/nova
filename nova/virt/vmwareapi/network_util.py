...
def get_dvswitch_for_vlan_interface(session, vlan_interface, cluster=None):
    """Gets the dvswitch associated with the physical network adapter
    with the name supplied.
    """
    # Get the list of dvSwicthes on the Host System
    host_mor = vm_util.get_host_ref(session, cluster)
    dvswitches_ret = session._call_method(vim_util,
                "get_dynamic_property", host_mor,
                "HostSystem", "config.network.proxySwitch")
    # Meaning there are no dvSwitches on the host. Shouldn't be the case,
    # but just doing code check
    if not dvswitches_ret:
        return
    dvswitches = dvswitches_ret.HostProxySwitch
    # Get the dvSwitch associated with the network adapter
    for elem in dvswitches:
        try:
            for nic_elem in elem.pnic:
                if str(nic_elem).split('-')[-1].find(vlan_interface) != -1:
                    return elem.dvsName
        # Catching Attribute error as a dvSwitch may not be associated with a
        # physical NIC.
        except AttributeError:
            pass
...
def create_port_group_all_hosts(session, pg_name, vswitch_name,
                                vlan_id=0, cluster=None):
    """Creates port groups on all hosts in the cluster with the vlan tags
    supplied. VLAN id 0 means no vlan id association.
    """
    client_factory = session._get_vim().client.factory
    add_prt_grp_spec = vm_util.get_add_vswitch_port_group_spec(
                    ...
                    pg_name,
                    vlan_id)
    host_mors = vm_util.get_hosts_ref(session, cluster)
...
def get_dvs_mor_by_name(session, vswitch_name):
    """Return DVS mor from its name
    """
    dvs_mors = session._call_method(
        vim_util, "get_objects", "DistributedVirtualSwitch", ["name"])
    for dvs_mor in dvs_mors.objects:
        if dvs_mor.propSet[0].val == vswitch_name:
            return dvs_mor.obj
    return None


def create_dvs_port_group(session, pg_name, vswitch_name,
                          vlan_id=0, cluster=None):
    """Creates a dvport group on the specified
    Distributed Virtual Switch with the vlan tags supplied.
    """
    LOG.debug("Entered create_dvs_port_group :"
              " (dvs_name, portgroup_name, vlanid) : (%s, %s, %s)" %
              (vswitch_name, pg_name, vlan_id))
    client_factory = session._get_vim().client.factory
    add_prt_grp_spec = vm_util.get_add_dvswitch_port_group_spec(
        client_factory, pg_name, vlan_id)
    dvs_mor = get_dvs_mor_by_name(session, vswitch_name)
